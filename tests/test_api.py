...
class PredictionApiTest(unittest.TestCase):
    ...
    def test_public_upcoming_matches_api_hides_paid_prediction_fields(self):
        ...
        first = payload["items"][0]
        self.assertEqual(first["status"], "scheduled")
        self.assertIn("kickoffUtc", first)
        self.assertTrue(first["kickoffUtc"].endswith("Z"))
        self.assertEqual(first["kickoffTimeZone"], "America/New_York")
    ...
    def test_public_match_summary_api_hides_paid_prediction_fields(self):
        ...
        self.assertIn("kickoffUtc", payload)
        self.assertTrue(payload["kickoffUtc"].endswith("Z"))
        self.assertEqual(payload["kickoffTimeZone"], "America/New_York")
